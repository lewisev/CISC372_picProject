...
void* convolute_process(void* data) {
    Data* localData = (Data*) data;

    Image* srcImage = localData->src;
    Image* destImage = localData->dest;
    double** algorithm = localData->algorithm;
    int rank = localData->rank;
...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
   // pthread setup
    ...
    Data* thread_data = (Data*) malloc(THREAD_COUNT*sizeof(Data));

    for(int thread = 0; thread < THREAD_COUNT; thread++) {
        thread_data[thread].src = srcImage;
        thread_data[thread].dest = destImage;
        thread_data[thread].algorithm = algorithm;
        thread_data[thread].rank = thread;

        pthread_create(&thread_handles[thread], NULL, &convolute_process, (void*) &thread_data[thread]);
    }
